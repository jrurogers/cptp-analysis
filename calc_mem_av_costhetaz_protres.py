...
"""
calc_mem_av_costhetaz_protres.py gro xtc memlip_costhetaz.txt
created by: Julia Rogers
created on: 11-9-22

calc av cos theta z of lipids w/heavy atom w/in 5 ang
of protein residue heavy atom


* note: must have already calculated cos theta z for each lipid *
...
def main():
   ...
   print "Calculating av cos theta z of lipids for each prot residue for gro %s and xtc %s" % (sys.argv[1], sys.argv[2])
   ...
   # get cos thetaz data
   costhetazs = np.reshape(np.loadtxt(sys.argv[3], usecols=(3,4)), (len(u.trajectory), nlip, 2))
   print(costhetazs.shape)
   print(costhetazs[0,0])

   res_vals = [[] for i in range(nres)]
   fname = 'protres_nearby_lipid_costhetaz_'+os.path.basename(sys.argv[2])[:-4]+'.txt'
      ...
      for k, t in enumerate(u.trajectory):
         ...
         for i in range(nres):
            ...
            for j in range(nlip):
               ...
               if mind <= 5:
                  curr.extend(costhetazs[k,j].flatten())
            res_vals[i].extend(curr)
            f.write('%8.4f %5i %8.4f\n' % (t.time/1000.0, i+1, np.mean(curr)))


   fname = 'memlip_av_costhetaz_protres_'+os.path.basename(sys.argv[2])[:-4]+'.txt'
   with open(fname, 'w') as f:
      for i in range(nres):
