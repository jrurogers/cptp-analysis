...
"""
calc_mem_av_scc_protres.py gro xtc memlip_scc.txt
created by: Julia Rogers
created on: 11-9-22

calc av Scc of lipids w/heavy atom w/in 5 ang
...
def main():
   ...
   print "Calculating av Scc of lipids for each prot residue for gro %s and xtc %s" % (sys.argv[1], sys.argv[2])
   ...
   # get scc data
   sccs = np.reshape(np.loadtxt(sys.argv[3], usecols=(2,)), (len(u.trajectory), nlip))
   print(sccs.shape)
   print(sccs[0,0])

   res_vals = [[] for i in range(nres)]
   fname = 'protres_nearby_lipid_scc_'+os.path.basename(sys.argv[2])[:-4]+'.txt'
      ...
      for k, t in enumerate(u.trajectory):
         ...
         for i in range(nres):
            ...
            for j in range(nlip):
               ...
               if mind <= 5:
                  curr.append(sccs[k,j])
            res_vals[i].extend(curr)
            f.write('%8.4f %5i %8.4f\n' % (t.time/1000.0, i+1, np.mean(curr)))


   fname = 'memlip_av_scc_protres_'+os.path.basename(sys.argv[2])[:-4]+'.txt'
   with open(fname, 'w') as f:
      for i in range(nres):
