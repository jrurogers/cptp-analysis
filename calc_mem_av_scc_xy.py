...
"""
calc_mem_av_scc_xy.py gro xtc memlip_scc.txt
created by: Julia Rogers
created on: 11-4-22

calc av coarse-grained order param (Scc) on grid in xy plane around protein
...
def main():
   ...
   print "Calculating Scc of lipids on xy grid for gro %s and xtc %s" % (sys.argv[1], sys.argv[2])
   ...
   xs = np.array(xs)
   ys = np.array(ys)
   print xs.shape, ys.shape, sccs.shape

   fname = 'memlip_av_scc_xy_'+os.path.basename(sys.argv[2])[:-4]+'.txt'
   with open(fname, 'w') as f:
      for i in range(len(xgrid)):
         ...
         for j in range(len(ygrid)):
            ...
            av = np.mean(sccs[xndxs & yndxs])
            st = np.std(sccs[xndxs & yndxs])
