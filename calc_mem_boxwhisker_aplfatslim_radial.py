...
"""
calc_mem_boxwhisker_aplfatslim_radial.py gro xtc apl_basename
created by: Julia Rogers
created on: 11-3-22

calc box whisker of area per lipid based on radial distance
...
def main():
   ...
   fname = 'memlip_boxwhisker_aplfatslimnointeract_radial_'+os.path.basename(sys.argv[2])[:-4]+'.txt'
   with open(fname, 'w') as f:
      for i in range(1, len(dgrid)):
         f.write('%8.4f' % dgrid[i])
         ndxs = (rs >= dgrid[i-1]) & (rs < dgrid[i])
         stats = calc_stats(apls[ndxs])
