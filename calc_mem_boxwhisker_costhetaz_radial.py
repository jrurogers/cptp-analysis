...
"""
calc_mem_boxwhisker_costhetaz_radial.py gro xtc memlip_costhetaz.txt
created by: Julia Rogers
created on: 11-3-22

calc box whisker of costhetaz based on radial distance

* note: must have already calculated costhetaz for each lipid *
...
def main():
   ...
   print "Creating box whisker of cos theta z for gro %s and xtc %s" % (sys.argv[1], sys.argv[2])
   ...
   fname = 'memlip_boxwhisker_costhetaz_radial_'+os.path.basename(sys.argv[2])[:-4]+'.txt'
   with open(fname, 'w') as f:
      for i in range(1, len(dgrid)):
         f.write('%8.4f' % dgrid[i])
         ndxs = (rs >= dgrid[i-1]) & (rs < dgrid[i])
         stats = calc_stats(thetas[ndxs].flatten())
