...
"""
calc_mem_boxwhisker_mindcc_ncc_radial.py gro xtc memlip_mindcc_ncc.txt
created by: Julia Rogers
created on: 11-3-22

calc box whisker of min dcc and ncc based on radial distance
...
def main():
   ...
   fname = 'memlip_boxwhisker_mindcc_ncc_radial_'+os.path.basename(sys.argv[2])[:-4]+'.txt'
   with open(fname, 'w') as f:
      for i in range(1, len(dgrid)):
         f.write('%8.4f' % dgrid[i])
         ndxs = (rs >= dgrid[i-1]) & (rs < dgrid[i])
         stats = calc_stats(mindccs[ndxs])
         for j in range(len(stats)):
            f.write('%14.6f' % stats[j])
         stats = calc_stats(nccs[ndxs])
         for j in range(len(stats)):
            f.write('%14.6f' % stats[j])
         f.write('\n')
